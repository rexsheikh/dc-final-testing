...
\begin{center}
{\LARGE \textbf{PDF-to-Anki Text Service Report}}\\[0.5em]
{\large Based on \texttt{ARCHITECTURE.md}}
\end{center}

\section{Software and Hardware Components}

The service is intentionally lightweight so that every tier can run on commodity Google Compute Engine VMs while keeping dependencies in the Python standard library plus a minimal set of supporting tools.
...
\addlinespace
REST VM hardware & \texttt{e2-medium} (2 vCPU, 4 GB RAM) & Hosts Flask + Redis + file storage under \texttt{/tmp}. \\
\addlinespace
...
\addlinespace
Storage & Local \texttt{/tmp/uploads}, \texttt{/tmp/outputs} & Designed to migrate to Cloud Storage once durability/scalability is required. \\
\addlinespace
...
\begin{enumerate}[itemsep=0.3em]
    \item Clients upload \texttt{.txt} files to the Flask REST tier, which stores them under \texttt{/tmp/uploads} and enqueues job IDs in Redis using \texttt{LPUSH}.
    \item Workers running on separate \texttt{f1-micro} VMs perform blocking pops (\texttt{BRPOP}) against the \texttt{job\_queue}, fetch the associated metadata, and read the text file via SSH-mounted paths or HTTP fetch from the REST VM.
    \item Each worker executes \texttt{nlp.py}: it calculates Flesch-Kincaid metrics, extracts keywords/entities, and now can emit complex-word CSV decks. Output decks are stored under \texttt{/tmp/outputs}.
...
\begin{itemize}[itemsep=0.2em]
    ...
    \item Filesystem storage under \texttt{/tmp/uploads} and \texttt{/tmp/outputs} keeps the architecture simple; future versions can swap in Cloud Storage without changing worker logic.
\end{itemize}
